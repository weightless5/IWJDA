...
class CSA(nn.Module):  # 定义一个继承自 nn.Module 的 EMA 类
    def __init__(self, channels, c2=None, factor=3):  # 构造函数，初始化对象
        super(CSA, self).__init__()  # 调用父类的构造函数
    ...
    def forward(self, x):  # 定义前向传播函数
        ...
        my_groups=x.reshape(b,self.groups, -1, h)
        t1=self.avg_pool(my_groups).view(b,-1)
        t2=self.max_pool(my_groups).view(b,-1)

        # x=my_groups*attention
        ...
        group_x=group_x.reshape(b,self.groups, -1, h)#计算权重
        group_x =  group_x
