...
def adaption_one_epoch(model_F, model_C, model_D2, target_loader, source_loader, target_val_loader, optimizer,
    ...
    for step in tqdm(range(max_iter), desc=f"Ep {epoch_idx}"):
        ...
        _, infer_feat = infer_model_F(tgt_imgs)
...
if __name__ == "__main__":
    ...
    for target_subj in range(num_subjects):
        ...
        subject_result = {
            ...
            'pretrain_best_acc': best_src_val_acc,
            'pretrain_best_precision': best_src_precision,
            'pretrain_best_recall': best_src_recall,
            'pretrain_best_f1': best_src_f1,
            'adapt_best_acc': best_test_acc,
            'adapt_best_precision': best_pre,
            'adapt_best_recall': best_recall,
            'adapt_best_f1': best_f1
        }
        results.append(subject_result)

    results_df = pd.DataFrame(results)
    save_path = os.path.join(result_dir, f"cross_subject_metrics_{save_time}.csv")
